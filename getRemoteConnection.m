...
if isempty(clusterHost)
    error('parallelexamples:GenericHTCondor:MissingAdditionalProperties', ...
...
if isempty(cluster.UserData)
    ...
    if ~isstruct(cluster.UserData)
        error('parallelexamples:GenericHTCondor:IncorrectUserData', ...
    ...
    if isfield(cluster.UserData, 'RemoteConnection')
        ...
        if isempty(remoteConnection)
            ...
            elseif cluster.HasSharedFilesystem && ...
                ...
                warning('parallelexamples:GenericHTCondor:DifferentRemoteParameters', ...
...
if useUI
    ...
    if isempty(usernameResponse)
        % User hit cancel
        error('parallelexamples:GenericHTCondor:UserCancelledOperation', ...
...
if useUI
    ...
    if strcmp(passphraseResponse, 'Cancel')
        % User hit cancel
        error('parallelexamples:GenericHTCondor:UserCancelledOperation', 'User cancelled operation.');
    end
