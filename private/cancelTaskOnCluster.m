...
%CANCELTASKONCLUSTER Cancels a task on the HTCondor scheduler
...
if ~strcmpi(task.Parent.Type, 'independent')
    OK = false;
    warning('parallelexamples:GenericHTCondor:FailedToCancelTask', ...
...
commandToRun = sprintf('condor_rm %s', schedulerID);
...
if cmdFailed && ~contains(cmdOut, {' not found', 'remove all jobs in cluster', 'no jobs in the queue'})
