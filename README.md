...
[![View on File Exchange](https://www.mathworks.com/matlabcentral/images/matlab-file-exchange.svg)](https://www.mathworks.com/matlabcentral/fileexchange/78823)
...
This repository contains MATLAB code files and shell scripts that you can use to submit jobs from a MATLAB or Simulink session running on Windows&reg;, Linux&reg;, or macOS to a HTCondor scheduler running on Linux.

## Products Required

- [MATLAB](https://mathworks.com/products/matlab.html) and [Parallel Computing Toolbox](https://mathworks.com/products/parallel-computing.html), release R2019a or newer, installed on your computer.
...
For example, the `condor_submit` command is used to submit work and `condor_q` to query the state of submitted jobs.
...
This is typically true if your MATLAB session is running on the same HTCondor cluster you want to submit to.
...
This submission mode is useful for submitting from a MATLAB session on a Windows computer to a Linux HTCondor cluster on the same network.
Your Windows machine creates an SSH session to the cluster head node to access the HTCondor utilities, while using a shared networked folder to store job data files.
...
```matlab
c.AdditionalProperties.ClusterHost = 'htcondor01.organization.com'; % MATLAB will SSH to htcondor01 to submit jobs
```
